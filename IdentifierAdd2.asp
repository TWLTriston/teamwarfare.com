...
function fValidateIdentifier(oForm) {
	...
	<%
	strSQL = "SELECT IdentifierName, IdentifierID, IdentifierFormat FROM tbl_identifiers ORDER BY IdentifierName ASC "
	...
	if (oRegEx.test(strIdentifier)) {
		...
		alert("Error:\n'" + strIdentifier + "' is not a valid " + oForm.txtIdentifierValue.text + ".");
	}
}
</script>
	<%
	...
	<tr>
		...
		<td bgcolor="<%=bgctwo%>">
			...
			<%
			strSQL = "SELECT IdentifierName, IdentifierID, IdentifierDesc FROM tbl_identifiers ORDER BY IdentifierName ASC "
			oRs.Open strSQL, oConn
			If Not(oRs.EOF AND oRs.BOF) Then
				Do While Not(oRs.EOF)
	...
	<tr>
		<td bgcolor="<%=bgcone%>" align="right"><b>Identifier Description:</b></td>
		<td bgcolor="<%=bgctwo%>">
		<%
			strSQL = "SELECT IdentifierDesc FROM tbl_identifiers WHERE IdentifierID = " & document.frmAntiSmurf.selIdentifierID.value
			oRs2.Open strSQL, oConn
			If Not(oRs.EOF AND oRs.BOF) Then
				Response.Write oRs2.Fields("IdentifierDesc").Value
			End If%>
		</td>
	</tr>
	<tr>
