...
<p class=small><b>Report Win: <% =Server.HTMLEncode (tlname) %> on the <%=Server.HTMLEncode(mname)%></b></p></td></tr>
<input type=hidden name=jdate value=<%=now()%>>

<input id=savetype name=AWin type=hidden value="Yes">
...
<td align=center>&nbsp;&nbsp;<INPUT type=submit value="Verify Report Win" id=submit name=submit></td></tr>
</table>
</FORM>
</td></tr>
</table>
