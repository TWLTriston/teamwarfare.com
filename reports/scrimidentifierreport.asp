...
<form name=ladder id=ladder method=get action=scrimidentifierreport.asp>
	<table align=center>
<TR><TD>Ladder:</TD><TD>
<select name=lname id=lname>
<%
strsql = "select EloLadderName, EloLadderID from tbl_elo_ladders where EloActive = 1 AND EloIdentifierID <> 0 order by EloLadderName asc"
ors.open strsql, oconn
if not(ors.eof and ors.bof) then
	do while not(ors.eof)
		Response.Write "<option value=""" & ors.fields("EloLadderName").value & """"
		if Request.QueryString("lname") = ors.fields("EloLadderName").value then
			Response.Write " selected "
		end if
		Response.Write ">" & ors.fields("EloLadderName").value & " Ladder</option>" & vbcrlf
		ors.movenext
	loop
end if
ors.nextrecordset
...
if Len(Request.QueryString("lname")) > 0 then
	strsql = "EXECUTE EloLadderReportCheckIdentifier @EloLadderName = '" & CheckString(Request.QueryString("lname")) & "'"
	ors.open strsql, oconn
