...
%>
	<table width="97%" border="0" cellspacing="0" cellpadding="0" bgcolor="#102C4A">
	<tr bgcolor=<%=bgcone%> height=25>
	<td width=100><p class=whitesmallText><b>&nbsp;Defender</b></p></td>
	<td width=100><p class=whiteLargeText><b>Attacker</b></p></td>
	<td width=100><p class=whiteLargeText><b>Result</b></p></td>
	<td><p class=whiteLargeText><b>Date</b></p></td>
	<td align=center><p class=whiteLargeText><b>Winner</b></p></td>
</tr>
<%	strSQL="select top 20 * from tbl_history where MatchLadderID='1' order by matchdate desc"
	...
	if not (ors.eof and ors.BOF) then
		do while not ors.EOF
			...
			%>
			<tr>
			<td><p class="goldSmallText">&nbsp;<a href="http://www.teamwarfare.com/viewteam.asp?team=<%=server.urlencode(defname)%>" onmouseover="(window.status='View team\'s profile'); return true" onmouseout="(window.status='<%=javatitle%>'); return true">#<%=defrank%>. <%=server.htmlencode(defname)%></a></p></td>
			<td><p class="goldSmallText"><a href="http://www.teamwarfare.com/viewteam.asp?team=<%=server.urlencode(attname)%>" onmouseover="(window.status='View team\'s profile'); return true" onmouseout="(window.status='<%=javatitle%>'); return true">#<%=attrank%>. <%=server.htmlencode(attname)%></a></p></td>
			<td><p class="goldSmallText"><font color="#C6C6FF" face="Arial" size="1"><%=result%></p></td><td><p class="goldSmallText"><font color="#C6C6FF" face="Arial" size="1"><%=ors.Fields("MatchDate").Value%></p></td><td align=center><p class=goldSmallText><font color="#C6C6FF" face="Arial" size="1">
			...
			<p class=goldSmallText><font color="#C6C6FF" face="Arial" size="1">&nbsp;<b><%=server.htmlencode(map1)%>:</b> <%=map1defscore%> - <%=map1attscore%><%
			if map1ot then
