...
%>
	...
	<br />
	<center><a href="/stuff/"><img src="/images/pad/banner.jpg" height="115" width="360" alt="Official TeamWarfare Mousepads - Learn More" border="0" /></a></center>
	<% Call Content2BoxEnd() %>
	<% 
					...
					if not(ors2.eof and ors2.bof) then
						...
						If oRs.Fields("IsAdmin").Value = 1 Then 
							...
							<tr>
								<td>Last Comm:</td>
							<%
							strSQL = "SELECT TOP 1 CommDate, CommTime WHERE RoundsID = '" & RoundsID & "' ORDER BY CommID DESC"
							oRs2.Open strSQL, oConn
							If Not(oRs.EOF AND ors.BOF) Then
								Response.Write "<td>" & ors2.Fields("CommDate").Value & " " & formatdatetime(ors2.Fields("CommTime").Value, 3) & "</td>"
							Else
								Response.Write "<td>No communications have been posted for this round.</td>"
							End If
						End If 
					End If
						%>
