...
<form name=ladder id=ladder method=get action=identifierreport.asp>
	<table align=center>
<TR><TD>Ladder:</TD><TD>
<select name=lname id=lname>
<%
strsql = "select LadderName, LadderID from tbl_ladders where laddershown = 1 AND IdentifierID <> 0 order by LadderName asc"
ors.open strsql, oconn
if not(ors.eof and ors.bof) then
	do while not(ors.eof)
		Response.Write "<option value=""" & ors.fields("LadderName").value & """"
		if Request.QueryString("lname") = ors.fields("LadderName").value then
			Response.Write " selected "
		end if
		Response.Write ">" & ors.fields("LadderName").value & "</option>" & vbcrlf
		ors.movenext
	loop
end if
ors.nextrecordset
...
if Len(Request.QueryString("lname")) > 0 then
	strsql = "EXECUTE LadderReportCheckIdentifier @LadderName = '" & CheckString(Request.QueryString("lname")) & "'"
	ors.open strsql, oconn
