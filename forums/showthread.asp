...
End If
Dim intContributor
strSQL = "SELECT tbl_threads.*, PlayerTitle, Contributor, PlayerSignature FROM tbl_Threads LEFT OUTER JOIN tbl_players on ThreadAuthorID=PlayerID WHERE ThreadID = '" & CheckString(intThreadID) & "'"
...
If Not(oRS.EOF and oRS.BOF) Then
	...
	strPageTitle = "TWL: " & Server.HTMLEncode(strThreadSubject)
	...
	If blnThreadEdited AND (strThreadEditBy = "Triston" OR strThreadEditBy = "Polaris" OR strThreadEditBy = "ZedsDead" OR strThreadEditBy = "TotalCarnage" OR strThreadEditBy = "Garath") Then
...
strsql = "SELECT tbl_posts.*, Contributor, PlayerID, PlayerTitle, playerHandle, PlayerSignature FROM tbl_posts INNER JOIN tbl_players ON PlayerID = PostAuthorID WHERE ThreadID = '" & CheckString(intThreadID) & "' ORDER BY PostID ASC"
				...
				if intPageNum = 1 then
					...
					<TR><TD><%
						...
						If blnSysAdmin or blnForumModerator Then
							Response.Write " / <a href=""forumengine.asp?SaveType=Thread&amp;mode=Delete&amp;threadid=" & intThreadID & "&amp;forumid=" & intForumID & """ onclick=""return confirm('Are you certain you want to delete this thread?')"">delete</a>"
						End If
					%></span>
					</td></tr>
					...
					do while not(ors.eof) AND intCounter < intPerPage
						...
						<table border=0 cellspacing=0 cellpadding=2 width=100% >
							...
							<%
								...
								End If
								If blnSysAdmin or blnForumModerator Then
									Response.Write " / <a href=""forumengine.asp?SaveType=Post&amp;mode=Delete&amp;threadid=" & intThreadID & "&amp;postid=" & oRS.Fields("PostID").Value & "&amp;forumid=" & intForumID & """ onclick=""return confirm('Are you certain you want to delete this post?')"">delete</a>"
								End If
							%></span>
							</td></tr>
							<TR><TD><hr class=forum /></TD></TR>
							...
							If oRS.Fields("PostEdited").Value AND (ors.fields("PostEditBy").value <> "Polaris" AND ors.fields("PostEditBy").value <> "Triston" AND ors.fields("PostEditBy").value <> "ZedsDead" AND ors.fields("PostEditBy").value <> "TotalCarnage" AND ors.Fields("PostEditBy").Value <> "Garath") Then
...
If blnSysAdmin Or blnForumModerator Then
	...
	If blnSysAdmin Then
		...
		If Not(orS.EOF AND oRS.BOF) Then
			...
			Loop
			%>
			</select>&nbsp;<input type="submit" value="Move It" />
			</form>
			<%
		End If
		oRS.Close
...
Function ListPages(byVal iPageNum, byVal iTotalPages, byVal iThreadID, byVal iForumID)
	Dim i
