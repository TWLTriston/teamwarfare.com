...
strPageTitle = "TWL: Scrim Ladder Admins"
...
<%
Call ContentStart("Assign Ladder Admins")
%>
<form name="frmLadderAdmin" id="frmLadderAdmin" action="saveitem.asp" method="post">
...
<tr>
	<th colspan="2" bgcolor="#000000">Current Ladder Admins</th>
</tr>
<tr>
	<td align="right" bgcolor="<%=bgcone%>">Admin:</td>
	<td bgcolor="<%=bgcone%>">
		<select name="selEloLadderAdminID" id="selEloLadderAdminID">
		<option value="">Select an Admin</option>
		<%
		strSQL = "SELECT p.PlayerHandle, p.PlayerID, lnk.EloLadderAdminID, l.EloLadderName "
		strSQL = strSQL & " FROM lnk_elo_admin lnk "
		strSQL = strSQL & " INNER JOIN tbl_players p ON lnk.PlayerID = p.PlayerID "
		strSQL = strSQL & " INNER JOIN tbl_elo_ladders l ON lnk.EloLadderID = l.EloLadderID "
		strSQL = strSQL & " ORDER BY p.PlayerHandle ASC "
		oRs.Open strSQL, oConn
		If Not(oRs.EOF AND oRs.BOF) Then
			Do While Not(oRs.EOF)
				%>
				<option value="<%=oRs.Fields("EloLadderAdminID").Value%>"><%=Server.HTMLENcode(oRs.Fields("PlayerHandle").Value) & " - " & Server.HTMLEncode(oRs.Fields("EloLadderName").Value) %></option>
				<%
				oRs.MoveNext
			Loop
		End If
...
<input type="hidden" name="SaveType" id="SaveType" value="RemoveAdmin" />
</form>
...
<table border="0" cellspacing="1" cellpadding="4" width="400">
	<% if request("player") <> "" Then %>
		...
		<tr bgcolor=<%=bgctwo%> height=30><td>&nbsp;&nbsp;Ladder</td><td><select name=LadderID class=bright style="width:200">
		<%
		strsql="select EloLadderName, EloLadderID from tbl_elo_ladders order by EloLadderName"
		ors.Open strsql, oconn
		if not (ors.EOF and ors.BOF) then
			do while not ors.EOF
				Response.Write "<option value=" & ors.Fields(1).Value & ">" & Server.HTMLEncode(ors.Fields(0).Value) & " Ladder</option>"
				ors.MoveNext
			loop
		end if
		ors.NextRecordSet
	...
	<tr bgcolor=<%=bgcone%>><td colspan=2 align=center><input type=submit value='Make it So' class=bright id=submit2 name=submit2>
		<input type=hidden name=SaveType value="AssignAdmin"></td>
	</tr>
	</form>
	</table>
</td></tr></table>
