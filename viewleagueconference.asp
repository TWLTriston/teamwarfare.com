...
<%
strSQL = "SELECT lnkLeagueTeamID, TeamName, TeamTag, LeaguePoints, Rank, Wins, Losses, Draws, NoShows, WinPct, RoundsWon, RoundsLost, DivisionName, d.LeagueDivisionID FROM "
...
strSQL = strSQL & " ON d.LeagueDivisionID = l.LeagueDivisionID "
strSQL = strSQL & " WHERE l.LeagueConferenceID = '" & intConferenceID & "' "
...
If Not(oRs2.EOF AND oRs2.BOF) Then
	...
	Do While Not (oRs2.EOF)
		...
		<tr>
			...
			<td bgcolor="<%=bgctwo%>"><a href="viewleaguedivision.asp?league=<%=Server.URLEncode(strLeagueName)%>&conference=<%=Server.URLEncode(strConferenceName)%>&division=<%=oRs2.Fields("DivisionName").Value %>"><%=Server.HTMLEncode(oRs2.Fields("DivisionName").Value & "")%> Division</a></td>
			<td bgcolor="<%=bgcone%>"><a href="viewteam.asp?team=<%=Server.URLEncode(orS2.Fields("TeamName").Value)%>"><%=Server.HTMLEncode(oRs2.Fields("TeamName").Value & " - " & oRS2.Fields("TeamTag").Value)%></a></td>
			<td bgcolor="<%=bgctwo%>"align="center"><%=oRs2.Fields("LeaguePoints").Value%></td>
			<td bgcolor="<%=bgcone%>"align="center"><%=oRs2.Fields("Wins").Value%></td>
			<td bgcolor="<%=bgctwo%>"align="center"><%=oRs2.Fields("Losses").Value%></td>
			<td bgcolor="<%=bgcone%>"align="center"><%=oRs2.Fields("Draws").Value%></td>
			<td bgcolor="<%=bgcone%>"align="center"><%=oRs2.Fields("NoShows").Value%></td>
			<td bgcolor="<%=bgctwo%>"align="center"><%=oRs2.Fields("RoundsWon").Value%></td>
			<td bgcolor="<%=bgcone%>"align="center"><%=oRs2.Fields("RoundsLost").Value%></td>
			<td bgcolor="<%=bgctwo%>"align="center"><%=FormatNumber(cInt(oRs2.Fields("WinPct").Value) / 10000, 3, 0)%></td>
...
Else
	%>
	<tr>
		<td bgcolor="<%=bgcone%>" colspan="10"><i>No teams are in this division</td>
	</tr>
	<%
End If
oRs2.NextRecordSet
