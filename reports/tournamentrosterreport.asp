...
<form name=ladder id=tournament method=get action=tournamentrosterreport.asp>
	<table align=center>
<TR><TD>Ladder:</TD><TD>
...
if Request.QueryString("tname") <> "" then
	...
	strSQL = "SELECT TeamName, Counter = count(distinct playerid), LastLogin, "
	strSQL = strSQL & " FounderJoin = (SELECT DateJoined FROM lnk_t_m_p tmp2 WHERE tmp2.PlayerID = tbl_teams.TeamFounderID AND TMLinkID = lnk_t_m.TMLinkID), TeamFounderID "
	strSQL = strSQL & " FROM lnk_T_m_p "
	strSQL = strSQL & " 	INNER JOIN lnk_T_M ON lnk_t_m.TMLinkID = lnk_t_m_p.TMLinkID "
	strSQL = strSQL & " 	INNER JOIN tbl_teams ON lnk_T_m.teamid = tbl_teams.teamid "
	strSQL = strSQL & " WHERE lnk_T_m.TournamentID= '" & Request.QueryString("tname") & "' "
	strSQL = strSQL & " GROUP BY lnk_T_m.tmlinkid, tbl_teams.teamname, LastLogin, TeamFounderID order by count(*) DESC"
