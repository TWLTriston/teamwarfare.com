...
%>
	...
	<td align=center width=100><p class=small><b>Result</b></p></td>
	<td><p class=small><b>Date</b></p></td>
	<td align=center><p class=small><b>Winner</b></p></td>
</tr>
<%	strSQL="select top 20 * from tbl_history where MatchLadderID='2' order by matchdate desc"
	...
	if not (ors.eof and ors.BOF) then
		do while not ors.EOF
			...
			attname = winnername
			defname = losername
			result = "<font color=red>Attacker Won</font>"
			If defwin then
				defname = winnername
				attname = losername
				result = "<font color=green>Defender Won</font>"
			end if				
			...
			%>
			<tr bgcolor=<%=bgctwo%>>
			<td><p class=small><a href="http://www.teamwarfare.com/viewteam.asp?team=<%=server.urlencode(defname)%>" onmouseover="(window.status='View team\'s profile'); return true" onmouseout="(window.status='<%=javatitle%>'); return true"><%=server.htmlencode(defname)%></a></p></td>
			<td><p class=small><a href="http://www.teamwarfare.com/viewteam.asp?team=<%=server.urlencode(attname)%>" onmouseover="(window.status='View team\'s profile'); return true" onmouseout="(window.status='<%=javatitle%>'); return true"><%=server.htmlencode(attname)%></a></p></td>
			...
			<p class=small>&nbsp;<b><%=map1%>:</b> <%=map1defscore%> - <%=map1attscore%><%
			if map1ot then
			...
			%><br>
			&nbsp;<b><%=map2%>:</b> <%=map2defscore%> - <%=map2attscore%><%
			if map2ot then
				response.write " in OT"
			end if
			if map2ft then
