...
strPageTitle = "TeamWarfare League&trade;: Community Based Gaming"
...
'Response.Flush
If blnLoggedIn Then
	strSQL = "EXECUTE PersonalizedHomePageAds '" & Session("PlayerID") & "'"
Else
	'game homepage hack.. eventually defaultgame should load gameonly news and ladder results for any game
    defaultgame = Request.QueryString
	If defaultgame = "tv" Then
		strSQL = "EXECUTE TVHomePage '0'"
	Else
	strSQL = "EXECUTE PersonalizedHomePageAds '0'"
	End If
End If
...
%>
	...
	If Not(oRS.EOF and oRS.BOF) Then
		...
		Do While Not(oRS.EOF)
			%>			
					<tr valign="top"> 
						<td bgcolor="<%=bgc%>"><%=oRs.Fields("GameAbbreviation").Value%>: <a href="#news<%=ors.fields("NewsID").value%>"><%=Server.HTMLEncode (ors.fields("NewsHeadLine").value)%></a></td>
...
%>
	<tr>
		<td align="right" colspan="2"><a href="/rss/">rss feeds</a> / <a href="allnews.asp">view all news</a>&nbsp;&nbsp;</td>
	</tr>
	</table>
		...
		<table width="100%" class="cssBordered">
			...
			If Not(oRS.EOF and oRS.BOF) Then
				...
				Do While Not(oRS.EOF)
					%>			
							<tr valign="top"> 
								<td bgcolor="<%=bgc%>"><a href="#news<%=ors.fields("NewsID").value%>"><%=Server.HTMLEncode (ors.fields("NewsHeadLine").value)%></a></td>
								<td bgcolor="<%=bgc%>"> <%=FormatDateTime(oRS.Fields("NewsDate").Value, vbShortDate) %> </td>				   
							</tr>
					<%
					if bgc = bgcone then
...
<% If False Then ' Disabled on 2/3/2012 angell 
	%>
<script type="text/javascript"><!--
...
//2007-02-23: Home Page
google_ad_channel = "6806303172";
...
If Session("LoggedIn") Then
						...
						If Not(oRs2.EOF AND oRs2.BOF) Then
							...
							Do While Not (oRs2.EOF)
								...
								<tr>
									...
									<% If cLng(oRs2.Fields("HomeTeamLinkID").Value) = oRs.Fields("lnkLeagueTeamID").Value Then %>
	...
	<% Call Content33BoxMiddle() %>
		...
		if strStatus="Attacking" then
						...
						Response.Write "<center><b>Current Status:</b> Attacking <a href=viewteam.asp?team=" & server.urlencode(strEnemyName) & ">"&Server.HTMLEncode(strEnemyName)&"</a> <br />Maps: "
						For i = 1 to intMaps
							If i > 1 Then
								Response.Write ", "
							End If
						...
						Next
						response.write "<br />Match Date: " & mdate & "</center><br /><br />"
						if matchdate1="TBD" then
							Response.Write "<center><b>Awaiting match acceptance from " & strEnemyName & "<br />(Challenged on " & xDate & ")</b></center>"
		...
		<table width="100%" class="cssBordered">
						...
						' End Map Selection Options
						Response.Write "<br /><table width=60% align=""center""><tr bgcolor="&bgcone&" height=35 valign=""middle""><td align=""center"">[ <a href=MatchReportLoss.asp?matchid=" & intMatchID & "&amp;teamid=" & intTeamID & ">Report Loss</a> ]&nbsp;&nbsp;&nbsp;&nbsp; "
...
'						Response.Write "<br /><table width=45% align=""center""><tr bgcolor="&bgcone&" height=35 valign=""middle""><td align=""center""><a href=MatchReportLoss.asp?matchid=" & intMatchID & "&amp;teamid=" & intTeamID & ">Report Loss</a></td></tr></table>"
					end if	
					strSQL = " SELECT COUNT(CommID) FROM tbl_comms WHERE MatchID = '" & intMatchID & "'"
					oRS2.Open strSQL, oConn
					If Not(oRS2.Eof and ORS2.BOF) Then
						Response.Write "<br /><br /><center><a href=""teamladderadmin.asp?team=" & Server.URLEncode(strTeamName) & "&amp;ladder=" & Server.URLEncode(strLadderName) & "#matchcomms"">Match Comms</a> (" & oRS2.Fields(0).Value & ")</center>"
					End If
					oRS2.NextRecordSet
	elseif strStatus="Defending" then
			...
			Response.Write "<center><b>Current Status:</b> Defending vs <a href=viewteam.asp?team=" & server.urlencode(strEnemyName) & ">"&Server.HTMLEncode(strEnemyName)&"</a><br />"
			Response.Write "Maps: "
			For i = 1 to intMaps
				If i > 1 Then
					Response.Write ", "
			...
			response.write " <br />Match Date: " & mdate & "</center><br /><br />"
			if  (matchdate1 = "TBD") then
				Response.Write "<center><a href=acceptmatch.asp?team=" & server.urlencode(strTeamName) & "&amp;ladder=" & server.urlencode(strLadderName) & "&amp;matchid=" & intMatchID & "&amp;enemy=" & server.urlencode(strEnemyName) & ">Accept the Challenge from " & Server.HTMLEncode(strEnemyName) & "</a><br />(You were challenged on " & xDate & ")</center>"
			else if mDate="TBD" then
				Response.Write "<center>You have chosen <b>" & matchdate1 & "</b> and <b>" & matchdate2 & "</b> for match dates.</center>"
				Response.Write "<center>Awaiting acceptance from " & Server.HTMLEncode(strEnemyName) & "(You were challenged on " & xDate & ")</center>"
			else
	...
	If oRs.State = 1 Then
		If Not(oRS.EOF and oRS.BOF) Then
			Do While Not(oRS.EOF)
				...
				End If
				%>
					<a id="news<%=ors.Fields("NewsId").Value %>"></a>
