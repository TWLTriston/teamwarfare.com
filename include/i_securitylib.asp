...
Function SecurityCheck(strSecurityFlagName)
	Dim bSecurityCheckPass, sSQL, objRSs
	If Len(Session("PlayerID")) = 0 Then
		bSecurityCheckPass = False
	Else
		CheckCookie()
		Set objRSs = Server.CreateObject("ADODB.RecordSet")
		bSecurityCheckPass = False
		sSQL = "SELECT lnkSecurityFlagID FROM lnk_Security_Flag WHERE UserID = '" & Session("PlayerID") & "' AND "
		sSQL = sSQL & "SecurityFlagName = '" & strSecurityFlagName & "'"
		objRSs.Open sSQL, oConn
		If Not(objRSs.EOF AND objRSs.BOF) Then
			bSecurityCheckPass = True
		End If
		objRSs.Close
		Set objRSs = Nothing
	End If
		return bSecurityCheckPass
End Function
