...
<%
Dim intPages, intTotalRecords
strSQL = "SELECT PlayerHandle, PlayerTitle, p.PlayerID, MRID, Rant, RantTime FROM tbl_match_rants mr INNER JOIN tbl_players p ON p.PlayerID = mr.PlayerID WHERE MatchID = '" & CheckString(intMatchID) & "' ORDER BY MRID ASC"
...
If (oRs.EOF AND oRs.BOF) Then
	%>
	<a href="LadderAddRant.asp?Ladder=<%=Server.URLEnCode(strLadderName & "")%>&MatchID=<%=intMatchID%>&cMode=add">Add Rant</a><br />
...
Else
	...
	Do While Not(oRS.EOF) AND oRs.AbsolutePage = intPageNum
		...
		%>
		<tr>
			<td valign="top" bgcolor="<%=bgc%>" width="15%"><b><%=Server.HTMLEncode(oRs.Fields("PlayerHandle").Value & "")%></b><br /><span class="usertitle"><%=oRs.Fields("PlayerTitle").Value%></span></td>
			<td bgcolor="<%=bgc%>">
				<%
