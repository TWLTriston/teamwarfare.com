...
If Not(bSysAdmin Or IsPlayerLadderAdmin(strLadderName)) Then
	oConn.Close 
	Set oConn = Nothing
	Set oRS = Nothing
	Response.Clear
...
strSQL = "SELECT PlayerLadderID FROM tbl_playerladders WHERE PlayerLadderName ='" & CheckString(strLadderName) & "'"
...
<input type=hidden name=SaveType value="MapListPlayer">
<% 
Call ContentStart("Definition of the map system")
%>
<TABLE WIDTH="97%" BORDER=0 CELLSPACING=0 CELLPADDING=4>
<TR>
	<TD>Player ladders only have 1 map per patch, so use the box below to configure the map list.</TD>
</TR>
</TABLE>
<%
Call ContentEnd()
...
%>
	...
	<td width=380 VALIGN=TOP>
		...
		<TR BGCOLOR="<%=bgcone%>">
			...
			strSQL = "SELECT m.MapName, m.MapID FROM tbl_maps m, lnk_pl_m lnk "
			strSQL = strSQL & " WHERE m.MapID = lnk.MapID AND PlayerLadderID = '" & intLadderID & "'"
			strSQL = strSQL & " ORDER BY m.MapName "
			oRs.Open strSQL, oConn
			if not (oRS.EOF and oRS.bof) then
				bgc=bgcone
		...
		<TR BGCOLOR=#000000>
			<TD ALIGN=CENTER>
		<input name=button type=button value='Save Map Assignments' class=bright ONCLICK="highlite(this.form.frm_current_maplist_map0);this.form.submit();">
			</TD>
		</TR>
		</TABLE>
		</TD></TR>
